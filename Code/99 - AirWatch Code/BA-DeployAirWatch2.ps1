...
$backupScriptPath = $WorkingDirectory + "\BA-BackupDatabase.sql"
...
$RemoteDropDirectory = "\\$ServerFqdn\Installers"
...
$7zExePath = $WorkingDirectory + "\7z\7za.exe"
...
function UnpackAndCopyArtifacts
{
	...
	# If available, print artifact information into deploy logs.
	$buildInfoPath = Join-Path -path $ArtifactDirectory -ChildPath "buildInfo.json"
	If (Test-Path $buildInfoPath)
	{
		...
		Write-Host "===================="
		Write-Host "Build Key: " $buildInfo."build-id"
		Write-Host "Commit ID: " $buildInfo."commit-id"
		Write-Host "===================="
	}
	...
	if ($RunMode -ne 5)
	{
		...
		& $7zExePath x $zipFileName
		if ($LastExitCode -ne 0)
		{
			Write-Host "Something went wrong during extraction of $zipFileName. Cannot continue."
			DeployExit $Session 21
		}
	}
	else
...
function BackupDatabase ($Session)
{
	...
	$ScriptBlock = [scriptblock]::Create("CMD /C sqlcmd -S $DbServer -U $DbUser -P $DbPass -d $DbSchema -b -i `'C:\Installers\BA-BackupDatabase.sql`'")
...
function ResetAdminUser ($Session)
{
	...
	$ScriptBlock = [scriptblock]::Create("CMD /C sqlcmd -S $DbServer -U $DbUser -P $DbPass -d $DbSchema -b -I -i `'C:\Installers\ResetAdminUserRoleAndOG.sql`'")
	Invoke-Command -session $Session -scriptblock $ScriptBlock
	$ExitCode = Invoke-Command -session $Session -scriptblock {$LastExitCode}
	if ($ExitCode -ne 0)
	{
...
function RestoreDatabase ($Session)
{
	...
	$ScriptBlock = [scriptblock]::Create("CMD /C sqlcmd -S $DbServer -U $DbSaUser -P $DbSaPass -d $DbSchema -b -I -i `'C:\Installers\RestoreDatabase.sql`'")
	Invoke-Command -session $Session -scriptblock $ScriptBlock
	$ExitCode = Invoke-Command -session $Session -scriptblock {$LastExitCode}
	if ($ExitCode -ne 0)
	{
...
function RestoreGoldenDbToIntegrationDb ($Session)
{
	...
	$ScriptBlock = [scriptblock]::Create("CMD /C sqlcmd -S $DbServer -U $DbSaUser -P $DbSaPass -d $DbSchema -b -I -i `'C:\Installers\ReplaceIntegrationDbWithGoldenDb.sql`'")
	Invoke-Command -session $Session -scriptblock $ScriptBlock
	$ExitCode = Invoke-Command -session $Session -scriptblock {$LastExitCode}
	if ($ExitCode -ne 0)
	{
...
function InstallDatabase ($Session, $SeedTestData)
{
	...
	if ($SeedTestData -eq 1)
	{
		...
		$ScriptBlock = [scriptblock]::Create("CMD /C $DbInstallerPath /s /V`"/qn /lie C:\Installers\DBInstall.log TARGETDIR=$InstallDir INSTALLDIR=$InstallDir AWPUBLISHLOGPATH=C:\Installers\DBPublish.log IS_SQLSERVER_AUTHENTICATION=1 IS_SQLSERVER_SERVER=$DbServer IS_SQLSERVER_USERNAME=$DbUser IS_SQLSERVER_PASSWORD=$DbPass IS_SQLSERVER_DATABASE=$DbSchema AWDEPLOYUNITTESTDATA=IntegrationTests`"")
	}
	else
	{
	...
	if ($ExitCode -eq 0 -or $ExitCode -eq 1)
	{
		...
		if ($ReadLogsOnSuccess -eq 1)
		{
			...
			Invoke-Command -session $Session -scriptblock {Get-Content "C:\Installers\DBInstall.log" | foreach {Write-Output $_}}
			Write-Host "=============================================================================================="
			Write-Host "=============================================================================================="
	...
	else
	{
		...
		$DoesPublishLogExist = Invoke-Command -session $Session -scriptblock {Test-Path "C:\Installers\DBPublish.log"}
		if ($DoesPublishLogExist -eq $True)
		{
			Write-Host "See publish logs below for additional information."
			Write-Host "=============================================================================================="
			Write-Host "=====================================DB PUBLISH LOG FILE======================================"
			$PublishLog = Invoke-Command -session $Session -scriptblock {(Get-Content "C:\Installers\DBPublish.log") | Out-String}
			Invoke-Command -session $Session -scriptblock {Get-Content "C:\Installers\DBPublish.log" | foreach {Write-Output $_}}
			Write-Host "=============================================================================================="
			Write-Host "=============================================================================================="
		}
		else
	...
	elseif ($DbInstallSuccessful -eq $False)
	{
		...
		$ScriptBlock = [scriptblock]::Create("Move-Item `"C:\Installers\DBPublish.log`" `"C:\Installers\DBPublish-$DbTimeOutRetryCount.log`"")
		$PublishLog = Invoke-Command -session $Session -scriptblock {$ScriptBlock}
...
function InstallAirwatch ($Session)
{
	...
	$ScriptBlock = [scriptblock]::Create("CMD /C $AppInstallerPath /s /V`"/qn /lie C:\Installers\AppInstall.log TARGETDIR=$InstallDir INSTALLDIR=$InstallDir AWIGNOREBACKUP=true AWSETUPCONFIGFILE=$ConfigFilePath`"")
	Invoke-Command -session $Session -scriptblock $ScriptBlock
	$ExitCode = Invoke-Command -session $Session -scriptblock {$LastExitCode}
	if ($ExitCode -eq 0 -or $ExitCode -eq 1)
	{
		...
		if ($ReadLogsOnSuccess -eq 1)
		{
			...
			Invoke-Command -session $Session -scriptblock {Get-Content "C:\Installers\AppInstall.log" | foreach {Write-Output $_}}
			Write-Host "=============================================================================================="
			Write-Host "=============================================================================================="
		}
	}
...
# Clean out contents of artifact directory on deploy agent.
function CleanAgentArtifactFolder
{
	Set-Location $ArtifactDirectory
	$Artifacts = Get-ChildItem $ArtifactDirectory
	Write-Host "Cleaning out $ArtifactDirectory"
	foreach ($Artifact in $Artifacts)
	{
		Remove-Item -force -recurse -confirm:$false $Artifact
	}
}
